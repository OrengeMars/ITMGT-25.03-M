...
with open(r"C:\Users\Chesca & Chesco\Desktop\New folder (2)\script.json","r") as file:
...
for key,value in tuple(jsondata.items()): #works
    jsondata.update({key.upper(): value.upper()}) #works

for key, value in jsondata.items():
    x = key.replace("", "")
    y = value.replace("<P>", "") and value.replace("</P>", "") and value.replace("<B>", "") and value.replace("</B>", "") and value.replace("<I>", "") and value.replace("<I>", "")
    jsondata[x] = y

for key, value in jsondata.items():
    x = key.replace("", "")
    y = value.replace("&", "") and value.replace("$", "") and value.replace("@", "") and value.replace(".", "") and value.replace(",", "") and value.replace("'", "") and value.replace("#", "") and value.replace("-", "") and value.replace("\", "") and value.replace(""", "") and value.replace("!", "") and value.replace("?", "") and value.replace("(", "") and value.replace(")", "") and value.replace("[", "") and value.replace("]", "") and value.replace("’", "") and value.replace("_", "") and value.replace(";", "") and value.replace(":", "") and value.replace("/", "")
    jsondata[x] = y
...
wordcount_dictionary = {'word', value}

for key in jsondata:
    if len(key) in jsondata >= 3:
        value += 1
        wordcount_dictionary[key] = word
...
df = pd.DataFrame(final_wordlist_of_tuples)
...
def Collatz(start_number):
    ...
    dictionary = { 'collatz_sequence', value, 
